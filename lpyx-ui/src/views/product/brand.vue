<template>
  <div class="tools-div">
    <el-button type="success" size="small" @click="handleAdd">添 加</el-button>
  </div>
    ...
    <el-form label-width="120px">
      ...
      <el-form-item label="品牌图标">
        <el-upload
          ...
          action="http://localhost:5050/admin/system/fileUpload"
...
const saveData = async () => {
  const { code, message } = await SaveBrand(brand.value)
...
const updateData = async () => {
  const { code, message } = await UpdateBrandById(brand.value)
  dialogVisible.value = false
  if (code === 200) {
    ElMessage.success('操作成功')
  } else {
